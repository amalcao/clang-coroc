...
struct Fixup {
  ...
  Fixup(LabelDecl *_LD, SourceRange &_SR) : LD(_LD), SR(_SR) { SS << "{\n\t"; }
};
...
class ThunkHelper {
  ...
  void dumpThunkFuncArgsType(std::ostream &OS, const FunctionProtoType *FuncTy);
...
public:
  ...
  void DumpThunkCallPrologue(std::ostream &OS, CallExpr *CE,
...
public:
  ...
  void DumpPrologueAndEpilogue() {
    if (!SimpleWay) {
      Prologue << "\t\t__chan_t __select_result_" << SelUID
               << " = __CoroC_Select(__select_set_" << SelUID << ", "
               << !HasDefault << ");\n";
    }
  ...
  void GenPrologueAndEpilogue() {
    ...
    Epilogue << "\n\t\t__CoroC_Select_Dealloc(__select_set_" << SelUID
             << ");\n\t";
  }
  ...
  void InsertCaseInitialization(BinaryOperator *BO) {
    ...
    if (SimpleWay)
      ...
      Prologue << "\t\t" << FuncName << "(__select_set_" << SelUID << ", ";
  ...
  bool RewriteCaseStmt(CoroCCaseStmt *Case) {
    ...
    SourceLocation SL = GetExpansionLoc(Rewrite, Case->getLocStart());
    SourceRange SR(SL);
...
class ScopeHelper {
  ...
  void EmitCleanupPerRef(std::stringstream &SS, ValueDecl *VD,
                         std::string Prefix) {
    ...
    if (Ty->isArrayType()) {
      ...
      SS << "__refcnt_put_array(" << Prefix << VD->getNameAsString() << ", "
         << ArrayTy->getSize().toString(10, false) << ");\n\t";
...
void ThunkHelper::dumpThunkStruct(std::ostream &OS) {
  ...
  for (; it != TheCallExpr->arg_end(); ++it) {
    OS << "\t" << (*it)->getType().getAsString() << " _param" << i++ << ";\n";
  }
  OS << "\tvoid *_fp;\n};\n";
}
...
void ThunkHelper::dumpThunkFunc(std::ostream &OS) {
  ...
  OS << "\nstatic int __thunk_helper_" << ThunkUID << "(";
  /**
    if (numArgs == 0)
    OS << "void*";
    else*/
...
void CoroCRecursiveASTVisitor::PopScopeStack() {
  ...
  if ((CurScope->ScopeFlags & SCOPE_GLOBAL) == 0 &&
    ...
    std::stringstream SS;
    SS << "\n\t/* cleanup code auto generated by clang-coroc : {{ */\n\t";
    CurScope->EmitCleanup(SS);
    SS << "/* }} */\n\t";

    Rewrite.InsertTextBefore(CurScope->SR.getEnd(), SS.str()); // !!
  }
  ScopeStack.pop_back();
}
...
void CoroCRecursiveASTVisitor::EmitFixupEnd(Fixup *fix) {
  Rewrite.InsertTextBefore(fix->SR.getBegin(), fix->SS.str());
  Rewrite.InsertTextAfterToken(getNextTokLocStart(fix->SR.getEnd()), "\n\t}");
...
void CoroCRecursiveASTVisitor::emitCleanupUntil(unsigned Flag, SourceRange SR,
  ...
  do {
    ...
    if (Scope->GetRefCnt() > 0) {
      Scope->EmitCleanup(SS, IgnoredVD);
      output = true;
    }
  ...
  Rewrite.InsertTextBefore(SR.getBegin(), SS.str());
...
void CoroCRecursiveASTVisitor::emitCleanupWithLabel(LabelDecl *S,
  ...
  if (output) {
    Rewrite.InsertTextBefore(SR.getBegin(), SS.str());
    Rewrite.InsertTextAfterToken(getNextTokLocStart(SR.getEnd()), "\n\t}");
  }
}
...
bool CoroCRecursiveASTVisitor::VisitValueDecl(ValueDecl *D) {
  ...
  if (!Ty->isPointerType() && 
    ...
    if (VD != nullptr && !isa<ParmVarDecl>(VD)) {
      if (VD->hasInit()) {
...
bool CoroCRecursiveASTVisitor::VisitCoroCSelectStmt(CoroCSelectStmt *S) {
  ...
  SelectHelper *SH =
      new SelectHelper(Context, Rewrite, SS.str(), CS->getLocStart(),
                       CS->getLocEnd(), CS->size(), hasDef);
